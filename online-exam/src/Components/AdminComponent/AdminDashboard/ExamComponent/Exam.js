...
// --------------------Adding Exam And re-render Exam component-----------------
   ...
   function handleInput(e){
    ...
    async function handleAddNewExam(){
        await axios.post("http://localhost:5000/exams", exam);
        setStatus(true);
    }
    ...
    // ----------------------------Deleting Exam-----------------------------------------------
       ...
       async function deleteExam(id){
        //    console.log(id);
            ...
            {
                if( parseInt( questions[i]._id) === parseInt( id )){
                    // console.log(questions[i].id);
                    await axios.delete(`http://localhost:5000/questions/${questions[i]._id}`);
                } 
            }
                                  ...
                                  exams.map((data ,i) => {
                                      return(
                                        <tr key={i}>
                                           ...
                                           <td>
                                               <NavLink exact to={`/AdminDashboard/Exam/Details/${data._id}`}>
                                                 <button>Details</button>  
                                               </NavLink> 

                                          <NavLink exact to={`/AdminDashboard/Exam/ViewQuestion/${data._id}`}>
                                                 <button>View Question</button>  
                                               </NavLink> 

                                             <NavLink exact to={`/AdminDashboard/Exam/AddQuestion/${data._id}`}>
